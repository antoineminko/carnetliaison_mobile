...
class _TeacherProfilePageState extends State<TeacherProfilePage> {
  ...
  Widget _buildSchoolCard(String name, String city, Color color) {
    return Container(
      width: 180, 
      ...
      child: Stack(
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13, color: AppTheme.textDark), overflow: TextOverflow.ellipsis),
                    Text(city, style: const TextStyle(color: AppTheme.textGrey, fontSize: 11, fontWeight: FontWeight.w500)),
                  ],
                ),
              ),
            ],
          ...
          Positioned(
            ...
            top: -5,
            child: IconButton(
              icon: Icon(Icons.cancel, color: Colors.red.withOpacity(0.7), size: 18),
              onPressed: () => _confirmSchoolRemoval(name),
    ...
    return GestureDetector(
      onTap: () {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Recherche d\'un nouvel établissement pour s\'affilier...'), backgroundColor: AppTheme.seaBlue)
        );
      },
      child: Container(
        width: 150,
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: AppTheme.seaBlue.withOpacity(0.2), style: BorderStyle.solid),
        ),
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, border: Border.all(color: AppTheme.seaBlue.withOpacity(0.1))),
              child: const Icon(Icons.add, color: AppTheme.seaBlue, size: 24),
            ),
            const SizedBox(height: 8),
            const Text('Ajouter', style: TextStyle(color: AppTheme.seaBlue, fontWeight: FontWeight.bold, fontSize: 12)),
          ],
        ),
      ),
    );
  ...
  void _confirmSchoolRemoval(String schoolName) {
    showDialog(
      context: context,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Demande de retrait de $schoolName envoyée.')),
              );
            },
